...
#### 任务 3.1.2：实现热钱包创建 API ✅
**文件**：`internal/api/handlers/wallet/post_create_hot_wallet.go`

**完成情况**：
- ✅ 实现 `PostCreateHotWalletRoute` 函数
- ✅ 实现 `postCreateHotWalletHandler` 函数
- ✅ 实现管理员权限检查（只有 admin 用户可以创建热钱包）
- ✅ 参数验证（chain_id 必填）
- ✅ 调用 HotWalletService.CreateHotWallet
- ✅ 错误处理（热钱包已存在等）
- ✅ 返回响应
- ✅ 注册路由
...
- ✅ 实现余额检查
- ✅ 实现费用计算（基础版）
- ✅ 实现热钱包余额检查（Native Token 和 ERC20 Token）
- ✅ 实现提现状态自动更新机制（UpdateWithdrawStatus）

#### 任务 3.2.2：实现提现状态自动更新 ✅
**文件**：`internal/wallet/withdraw/service.go`

**完成情况**：
- ✅ 实现 `UpdateWithdrawStatus` 方法
- ✅ 根据交易确认数自动更新提现状态（pending → processing → confirmed）
- ✅ 通过 RPC 查询交易回执并创建 transactions 记录（如果不存在）
- ✅ 集成到区块扫描器的 post-scan hooks 中

### 3.3 风控集成 ✅

#### 任务 3.3.1：实现管理员审核流程 ✅
**文件**：`internal/wallet/withdraw/service.go`, `internal/api/handlers/wallet/post_approve_withdraw.go`, `post_reject_withdraw.go`

**完成情况**：
- ✅ 实现 `ApproveWithdraw` 方法（管理员批准提现）
- ✅ 实现 `RejectWithdraw` 方法（管理员拒绝提现）
- ✅ 实现提现请求初始状态为 `user_withdraw_request`（等待审核）
- ✅ 实现 `ProcessWithdraw` 状态检查（只有 `user_withdraw_request` 状态可以处理）
- ✅ 实现失败提现的手动拒绝（支持拒绝 `failed` 状态的提现）
- ✅ 实现错误消息管理（管理员拒绝时覆盖系统错误消息）
- ✅ 实现 `PostApproveWithdrawRoute` API（管理员批准提现）
- ✅ 实现 `PostRejectWithdrawRoute` API（管理员拒绝提现）
- ✅ 实现管理员权限检查
- ✅ 注册路由
...
- ✅ 实现交易签名和发送
- ✅ 实现提现确认机制
- ✅ 实现状态流转：`user_withdraw_request` → `pending` → `processing` → `confirmed`
...
- ✅ 实现批量归集（自动循环所有用户钱包）
- ✅ 实现归集交易签名、广播与链上回执跟踪
- ✅ 实现 ERC20 归集时的 Gas 充值逻辑（ensureNativeGas）
- ✅ 实现归集顺序优化（先归集 ERC20，再归集 Native Token，提高 Gas 效率）
...
- ✅ 实现管理员权限检查（手动归集仅管理员可用）
- ✅ 实现管理员查询所有归集记录（不限制于自己的钱包）

### 5.4 全局配置 ✅

#### 任务 5.4.1：实现全局配置管理 ✅
**文件**：`internal/config/server_config.go`, `cmd/server/wallet_init.go`

**完成情况**：
- ✅ 实现 `Wallet` 配置结构（EnableAutoCollect, EnableSigning）
- ✅ 实现环境变量配置加载（`WALLET_ENABLE_AUTO_COLLECT`, `WALLET_ENABLE_SIGNING`）
- ✅ 实现自动归集开关（根据配置决定是否启动自动归集）
- ✅ 实现签名功能开关（根据配置决定是否允许签名）
- ✅ 默认值均为 `false`（安全优先）
...
- **阶段一：基础架构** - 100%
- **阶段二：充值模块** - 100%
- **阶段三：提现模块** - 95% (完成核心流程、管理员审核、状态自动更新，剩余部分测试)
- **阶段四：余额管理** - 90% (完成核心查询，剩余历史查询)
- **阶段五：归集和调度** - 100% (完成归集、调度、API、全局配置)
...
- ✅ **状态自动更新**：提现状态根据交易确认数自动更新（pending → processing → confirmed）
- ✅ **交易记录自动创建**：通过 RPC 查询交易回执，自动创建 transactions 记录
- ✅ **失败提现处理**：支持管理员手动拒绝失败的提现请求

### 归集模块优化
- ✅ **Gas 管理**：ERC20 归集时自动检查并充值 Native Token 作为 Gas
- ✅ **归集顺序优化**：先归集 ERC20，再归集 Native Token，提高 Gas 效率
- ✅ **管理员权限**：手动归集仅管理员可用，管理员可查询所有归集记录

### 全局配置
- ✅ **自动归集开关**：通过 `WALLET_ENABLE_AUTO_COLLECT` 环境变量控制
- ✅ **签名功能开关**：通过 `WALLET_ENABLE_SIGNING` 环境变量控制
- ✅ **安全优先**：默认值均为 `false`，需要显式启用
