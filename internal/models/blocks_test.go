...
func testBlocksExists(t *testing.T) {
	...
	e, err := BlockExists(ctx, tx, o.Hash, o.ChainID)
	if err != nil {
...
func testBlocksFind(t *testing.T) {
	...
	blockFound, err := FindBlock(ctx, tx, o.Hash, o.ChainID)
	if err != nil {
		t.Error(err)
	}
...
var (
	blockDBTypes = map[string]string{`Hash`: `character varying`, `ChainID`: `integer`, `ParentHash`: `character varying`, `Number`: `bigint`, `Timestamp`: `bigint`, `Status`: `enum.block_status('confirmed','safe','finalized','orphaned')`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`}
	_            = bytes.MinRead
)
